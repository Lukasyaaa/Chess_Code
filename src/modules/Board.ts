...
class Board{
    ...
    private canCastling(kingCell : Cell){
        const king : Figure | null = kingCell.getFigure();
        if(king){
            //Если Король Не под Шахом
            if(this.check !== king.getColor()){
                ...
                for(const move of this.history){
                    if(move.figure.type === FiguresType.King && move.figure.color === king.getColor()){
                        canShortCastling = false;
                        canLongCastling = false;
                        break;
                    }
                    if(move.figure.type === FiguresType.Rook && move.figure.color === king.getColor()){
                ...
                for(let dir : number = 1, can : number = 0; dir !== -3; dir-=2, can++){
                    if(canCastlings[can]){
                        ...
                        const enemyColor : Colors = Number(!Boolean(king.getColor()));
    ...
    private canEnPassanat(pawnCell : Cell){
        const pawn : Figure | null = pawnCell.getFigure();
        if(pawn){
            const pawnDir : number = ((!pawn.getColor()) ? 1 : -1);
                ...
                for(let xAggresive : number = 0; xAggresive < 8; xAggresive++){
                    ...
                    if(aggresiveCell.getFigure()?.getColor() === currentPlayer){
                        ...
                        if(checkableKing.isAvailable()){
                            ...
                            const startX : number = ((checkableKing.getX() - 1 >= 0) ? checkableKing.getX() - 1 : checkableKing.getX());
                            const endX   : number = ((checkableKing.getX() + 1 < 8) ? checkableKing.getX() + 2 : checkableKing.getX() + 1);
                            const startY : number = ((checkableKing.getY() - 1 >= 0) ? checkableKing.getY() - 1 : checkableKing.getY());
